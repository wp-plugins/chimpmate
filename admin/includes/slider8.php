...
height: 718px;
display: block;
background-color: {{data.stopt.slider_canvas_c||'#333'}};
position: relative;
}
...
text-align: center;
margin:0 50px;
background:  {{data.stopt.slider_bg_c||'#262E43'}} no-repeat;
...
padding-top:20px;
color: #fff;
color: {{data.stopt.slider_heading_fc||'#fff'}};
font-size: {{data.stopt.slider_heading_fs||'20'}}px;
line-height: {{data.stopt.slider_heading_fs||'20'}}px;
font-weight: {{data.stopt.slider_heading_fw}};
font-family: {{data.stopt.slider_heading_f | livepf}};
font-style: {{data.stopt.slider_heading_fst}};
}
#wpmchimpas .slider_msg, #wpmchimpas .slider_msg *{
color: #ADBBE0;
font-size: {{data.stopt.slider_msg_fs||'12'}}px;
font-family: {{data.stopt.slider_msg_f | livepf}};
}
...
outline:0;
display: block;
color: {{data.stopt.slider_tbox_fc||'#353535'}};
font-size: {{data.stopt.slider_tbox_fs||'17'}}px;
font-weight: {{data.stopt.slider_tbox_fw}};
font-family: {{data.stopt.slider_tbox_f | livepf}};
font-style: {{data.stopt.slider_tbox_fst}};
background-color: {{data.stopt.slider_tbox_bgc||'#fff'}};
width: {{data.stopt.slider_tbox_w}}px;
height: {{data.stopt.slider_tbox_h||'40'}}px;
line-height: {{data.stopt.slider_tbox_h||'40'}}px;
border: {{data.stopt.slider_tbox_bor||'1'}}px solid {{data.stopt.slider_tbox_borc||'#efefef'}};
...
box-shadow: 0 0 1px 1px {{data.stopt.slider_check_borc||'#ccc'}};
}
#wpmchimpas .slider_check .ctext{
color: {{data.stopt.slider_check_fc||'#fff'}};
...
content: {{data.stopt.slider_check_shade | chshade}};
margin: -4px;
display: block;
}
...
text-align: center;
cursor: pointer;
color: {{data.stopt.slider_button_fc||'#fff'}};
font-size: {{data.stopt.slider_button_fs || "17"}}px;
font-weight: {{data.stopt.slider_button_fw}};
font-family: {{data.stopt.slider_button_f | livepf}};
font-style: {{data.stopt.slider_button_fst}};
background-color:{{data.stopt.slider_button_bc||'#73C557'}};
width: {{data.stopt.slider_button_w}}px;
height: {{data.stopt.slider_button_h||'42'}}px;
line-height: {{data.stopt.slider_button_h||'42'}}px;
-webkit-border-radius: {{data.stopt.slider_button_br||'3'}}px;
-moz-border-radius: {{data.stopt.slider_button_br||'3'}}px;
border-radius: {{data.stopt.slider_button_br||'3'}}px;
border: {{data.stopt.slider_button_bor||'1'}}px solid {{data.stopt.slider_button_borc||'#50B059'}};
}
#wpmchimpas .slider_button:hover{
color: {{data.stopt.slider_button_fch}};
background-color: {{data.stopt.slider_button_bch||'#50B059'}};
}
...
content:"{{data.stopt.slider_soc_head||'Subscribe with'}}";
width: 100%;
display: block;
margin: 15px auto 5px;
color: {{data.stopt.slider_soc_fc||'#ADACB2'}};
font-size: {{data.stopt.slider_soc_fs||'13'}}px;
line-height: {{data.stopt.slider_soc_fs||'13'}}px;
font-weight: {{data.stopt.slider_soc_fw}};
font-family: {{(data.stopt.slider_soc_f | livepf)}};
font-style: {{data.stopt.slider_soc_fst}};
}
...
border-radius: 2px;
cursor: pointer;
border:1px solid {{data.stopt.slider_bg_c || '#262E43'}};
}
...
background-image: <?php echo ChimpMate_WPMC_Assistant::getIcon('fb',20,'#fff');?>
}
#wpmchimpas .wpmchimpa-social .wpmchimpa-soc.wpmchimpa-fb:hover:before {
background-image: <?php echo ChimpMate_WPMC_Assistant::getIcon('fb',20,'#2d609b');?>
}
#wpmchimpas .wpmchimpa-social .wpmchimpa-soc.wpmchimpa-gp::before {
background-image: <?php echo ChimpMate_WPMC_Assistant::getIcon('gp',20,'#fff');?>;
}
#wpmchimpas .wpmchimpa-social .wpmchimpa-soc.wpmchimpa-gp:hover:before {
background-image: <?php echo ChimpMate_WPMC_Assistant::getIcon('gp',20,'#eb4026');?>;
}
#wpmchimpas .wpmchimpa-social .wpmchimpa-soc.wpmchimpa-ms::before {
background-image: <?php echo ChimpMate_WPMC_Assistant::getIcon('ms',20,'#fff');?>
}
#wpmchimpas .wpmchimpa-social .wpmchimpa-soc.wpmchimpa-ms:hover:before {
background-image: <?php echo ChimpMate_WPMC_Assistant::getIcon('ms',20,'#00BCF2');?>
}
...
#slider_tag *{
pointer-events: none;
color: {{data.stopt.slider_tag_fc||'#68728D'}};
font-size: {{data.stopt.slider_tag_fs||'10'}}px;
font-weight: {{data.stopt.slider_tag_fw}};
font-family:Arial;
font-family: {{data.stopt.slider_tag_f | livepf}};
font-style: {{data.stopt.slider_tag_fst}};
}
#slider_tag:before{
content:{{getIcon('lock1',data.stopt.slider_tag_fs||10,data.stopt.slider_tag_fc||'#68728D')}};
margin: 5px;
top: 1px;
position: relative;
}
...
background: {{data.stopt.slider_trigger_bg || '#262E43'}};
}
#wpmchimpas-trig:before{ 
content:{{getIcon('m3',32,data.stopt.slider_trigger_c||'#fff',25.73)}};
height: 32px;
width: 32px;
display: block;
margin: 8px;
...
<div id="wpmchimpas">
<div class="wpmchimpas-inner" ng-class="{'wosoc':data.stopt.slider_dissoc}">
  <div class="wpmc-live-sc" ng-click="gotos($event)" data-optno="9" data-lhint="Go to Additional Theme Options" style="margin:-15px">8</div>
        <div><div class="wpmc-live-sc" ng-click="gotos($event)" data-optno="1" data-lhint="Go to Custom Message Settings">1</div>
            <h3>{{data.stopt.slider_heading}}</h3>
            <div class="slider_msg"><p ng-bind-html="data.stopt.slider_msg | safe"></p></div>
        </div>
        <div class="wpmchimpas">
          ...
          <div id="slider_tag" ng-bind-html="data.stopt.slider_tag||'Secure and Spam free...' | safe"></div></div>
          </div>
