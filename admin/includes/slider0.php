...
height: 718px;
display: block;
background: {{data.stopt.slider_bg_c||'#333'}};
color:#fff;
position: relative;
}
...
#wpmchimpas h3{
color: {{data.stopt.slider_tbox_fc||'#fff'}};
font-size: {{data.stopt.slider_tbox_fs||'18'}}px;
...
margin-bottom: 15px;
line-height: 20px;
font-size: {{data.stopt.slider_msg_fs||'14'}}px;
font-family:Arial;
...
border-radius: 3px;
{{data.stopt.slider_check_borc? '-webkit-box-shadow: 0 0 1px 1px '+data.stopt.slider_check_borc+';-moz-box-shadow: 0 0 1px 1px '+data.stopt.slider_check_borc+';-ms-box-shadow: 0 0 1px 1px '+data.stopt.slider_check_borc+';-o-box-shadow: 0 0 1px 1px '+data.stopt.slider_check_borc+';box-shadow: 0 0 1px 1px '+data.stopt.slider_check_borc+';' :''}}
}


#wpmchimpas .slider_check .cbox.checked {
background-color: {{data.stopt.slider_check_c||'#158EC6'}};
}
...
color: {{data.stopt.slider_tbox_fc}};
font-size: {{data.stopt.slider_tbox_fs||'14'}}px;
font-weight: {{data.stopt.slider_tbox_fw}};
font-family: {{data.stopt.slider_tbox_f | livepf}};
font-family:Arial;
font-style: {{data.stopt.slider_tbox_fst}};
...
border: {{data.stopt.slider_tbox_bor||'1'}}px solid {{data.stopt.slider_tbox_borc||'#dddddd'}};
}
...
font-size: {{data.stopt.slider_button_fs||'16'}}px;
...
{{data.stopt.slider_button_bc? "background-color:"+data.stopt.slider_button_bc+";" : "background: -moz-linear-gradient(left, #62bc33 0%, #8bd331 100%);
background: -webkit-gradient(linear, left top, right top, color-stop(0%,#62bc33), color-stop(100%,#8bd331));
background: -webkit-linear-gradient(left, #62bc33 0%,#8bd331 100%);
background: -o-linear-gradient(left, #62bc33 0%,#8bd331 100%);
background: -ms-linear-gradient(left, #62bc33 0%,#8bd331 100%);
...
border: {{data.stopt.slider_button_bor||'0'}}px solid {{data.stopt.slider_button_borc}};
}
#wpmchimpas .wpmchimpa-subs-button:hover{
background:#8BD331;
color: {{data.stopt.slider_button_fch||'#fff'}};
background-color: {{data.stopt.slider_button_bch}};
{{data.stopt.slider_button_bch? "background-color:"+data.stopt.slider_button_bch+";" : "background: -moz-linear-gradient(left, #8BD331 0%, #8bd331 100%);
...
border: 3px solid {{data.stopt.slider_spinner_c||'#fff'}};
border-radius: 30px;
...
background: {{data.stopt.slider_trigger_bg || '#000'}};
}
#wpmchimpas-trig:before{ 
content:{{getIcon('m1',32,data.stopt.slider_trigger_c||'#fff')}};
height: 32px;
width: 32px;
display: block;
margin: 8px;
...
<div class="wpmc-live-sc" ng-click="gotos($event)" data-optno="9" data-lhint="Go to Additional Theme Options" style="top:0">7</div>
  ...
  <div><div class="wpmc-live-sc righthov" ng-click="gotos($event)" data-optno="4" data-lhint="Go to Button Settings" style="right:0;">4</div>
    <div class="wpmchimpa-subs-button">{{data.stopt.slider_button}}</div>
  </div>
